...
// Server Action responsável por processar o login
import { loginAction } from "@/actions/autenticar";
        ...
        <Card className="bg-app-card border border-app-border w-full max-w-md mx-auto">
            ...
            <CardContent>
                <form className="space-y-4" action={formAction}>
                    ...
                    {state?.error && (
                        <div className="text-red-500 text-sm bg-red-50 p-3 rounded-md">{state.error}</div>
                    )}


                    <p className="text-center text-white">
                        Ainda não possui uma conta?{" "}
                        <Link href="/register" className="text-brand-primary hover:underline font-bold">Cadastre-se</Link>
                    </p>
